...
# 
# @param: a string
# @return: that string in hashtag format 
# 

def generate_hashtag(s):
    # return false immediately if string is unworkable in context of problem requirements    
    if s == None or s == '':
        return False
    # capitalise all words within the string - requirement 2  
    s = s.title()
    # remove all white space
    s = ''.join(s.split())
    # insert hashtag in first position - requirement 1     
    s = '#' + s
    # return false if string is greater than 140 characters - requirement 3
