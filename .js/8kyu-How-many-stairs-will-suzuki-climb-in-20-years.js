...
// 
// @param: 2D array of steps on each day of the week
// @return: treat this complete 2D array as a full year, return how many steps in 20 years
// 

function stairsIn20(s){
    const sunday = s[0].reduce((cur,acc) => cur+acc)
    const monday = s[1].reduce((cur,acc) => cur+acc)
    const tuesday = s[2].reduce((cur,acc) => cur+acc)
    const wednesday = s[3].reduce((cur,acc) => cur+acc)
    const thursday = s[4].reduce((cur,acc) => cur+acc)
    const friday = s[5].reduce((cur,acc) => cur+acc)
    const saturday = s[6].reduce((cur,acc) => cur+acc)
